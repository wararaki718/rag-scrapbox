...
async def encode(request: EncodeRequest, encoder: SpladeEncoder = Depends(get_encoder)):
    try:
...
async def encode_debug(request: EncodeRequest, encoder: SpladeEncoder = Depends(get_encoder)):
    try:
